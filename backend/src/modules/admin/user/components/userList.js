...
const UserList = () => {
  ...
  const { data, loading, error } = useFetch(`/api/users/getallUsers?page=${currentPage}&pageSize=${recordsPerPage}&refresh=${refresh}`);
  ...
  const handleEditUser = (userId) => {
    navigate(`${pageURL.addEditUser}?userId=${userId}`);
  };
  ...
  const handleConfirmDelete = async () => {
    try {
      await axios.delete(`/api/users/${currentSelectedUser.id}`);
  ...
  return (
    <div className="userListWrapper">
      <Header size="medium">Manage Users</Header>
